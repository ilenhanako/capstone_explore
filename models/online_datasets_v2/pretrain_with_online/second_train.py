...
def analyze_class_distribution(dataset_path: Path):
    ...
    for split in ["train", "val", "test"]:
        ...
        split_stats = {"files": 0, "labels": 0, "class_counts": Counter()}
    ...
    for class_id in sorted(stats["class_distribution"].keys()):
        ...
        percentage = (
            (count / stats["total_labels"] * 100) if stats["total_labels"] > 0 else 0
        )
        ...
        print(
            f"  Class {class_id} ({class_name:.<30}): {count:>6} ({percentage:>5.2f}%){status}"
        )
...
def verify_dataset_structure(dataset_path: Path) -> bool:
    ...
    for split in ["train", "val", "test"]:
        ...
        # Store stats
        stats[split] = {"images": len(img_files), "labels": len(lbl_files)}
    ...
    for split, counts in stats.items():
        print(
            f"   {split:>5}: {counts['images']:>5} images, {counts['labels']:>5} labels"
        )
...
def estimate_batch_size(model_name: str, imgsz: int, vram_gb: float) -> int:
    ...
    estimated = int(usable_mem / per_image_mem)  # estimated batch size
...
def train_model(
    ...
    if device == "auto":
        if torch.cuda.is_available():
            ...
            if batch == -1:
                model_size = Path(weights).stem  # Extract model name
    ...
    train_args = {
        ...
        # Resume training
        "resume": resume,
        # Add all optimized hyperparameters
        **OPTIMIZED_HPARAMS,
    }
    ...
    if verbose:
        ...
        print(
            f"     - HSV: h={train_args['hsv_h']}, s={train_args['hsv_s']}, v={train_args['hsv_v']}"
        )
...
Examples:
  ...
  python train_road_defects.py --dataset ./mergedDataset --mode fresh --epochs 10 --batch 4
    ...
    # Model configuration
    parser.add_argument(
        "--weights", default="yolov11m.pt", help="Pretrained weights (.pt file)"
    )
    ...
    parser.add_argument(
        "--batch", type=int, default=-1, help="Batch size (-1 for auto-detection)"
    )
    ...
    parser.add_argument("--device", default="auto", help="Device specification")
    ...
    parser.add_argument(
        "--save_period", type=int, default=10, help="Save checkpoint every N epochs"
    )
    ...
    if args.mode == "resume":
        ...
        if weights_path.exists():
            ...
            # Assume it's a model name (e.g., 'yolov11m.pt')
    ...
    try:
        ...
        print(
            f"     model = YOLO('{args.project_root}/{args.project_name}/weights/best.pt')"
        )
        ...
        print(
            f"   Resume with: python {sys.argv[0]} --dataset {args.dataset} --mode resume"
        )
        sys.exit(130)
